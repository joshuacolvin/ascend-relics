...
public class LockCommand implements CommandExecutor, TabCompleter {
    ...
    public LockCommand(AscendRelics plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length < 1) {
            MessageUtil.error(player, "Usage: /lock <relic>");
            return true;
        }
        ...
        if (plugin.lockManager().lock(type)) {
            plugin.dataManager().saveLocks(plugin.lockManager());
            MessageUtil.success(player, type.displayName() + " Relic has been locked.");
        } else {
            MessageUtil.info(player, type.displayName() + " Relic is already locked.");
        }
        return true;
    }
