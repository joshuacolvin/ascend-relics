...
public class WaterRelic extends Relic {
    ...
    private class WhirlpoolAbility extends ActiveAbility {
        WhirlpoolAbility() {
            super("Whirlpool", "Create a vortex that pulls enemies in", 15);
        }
    ...
    private class TidalShieldAbility extends ActiveAbility {
        TidalShieldAbility() {
            super("Tidal Shield", "Reduce incoming damage by 40% for 6s", 25);
        }

        @Override
        public AbilityResult execute(Player player, Plugin pluginRef) {
            player.addPotionEffect(new PotionEffect(
                    PotionEffectType.RESISTANCE, 120, 1, false, true, true));
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ticks++;
                    if (ticks > 120 || !player.isOnline()) { cancel(); return; }
                    angle += 0.3;
